...
export default function Capabilities() {
  ...
  return (
    <section
      id="capabilities"
      ref={containerRef}
      className="relative py-24 lg:py-36 bg-[#f8f9fc] dark:bg-black border-t border-black/5 dark:border-white/5 transition-colors duration-200"
    >
      {/* Ambient background glow */}
      <div className="absolute top-1/2 left-1/3 -translate-x-1/2 -translate-y-1/2 w-[550px] h-[350px] bg-[#1964eb]/5 dark:bg-[#2f7bff]/8 blur-[140px] pointer-events-none rounded-full" />
      ...
      <div className="portfolio-container relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 lg:mb-24 gap-6">
          <div>
            ...
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-zinc-900 dark:text-white leading-[1.08]">
              Digital systems engineered <br />
              <span className="text-zinc-500 dark:text-zinc-400">to move businesses forward.</span>
            </h2>
          </div>
        ...
        <div className="hidden lg:grid grid-cols-12 gap-12 items-start">
              ...
              return (
                <button
                  ...
                  className={`text-left p-6 rounded-2xl transition-all duration-300 border cursor-pointer group focus:outline-none focus:ring-2 focus:ring-[#2f7bff]/50 relative ${
                    isActive
                      ? "bg-white dark:bg-[#09090e] border-[#1964eb]/40 dark:border-[#2f7bff]/40 shadow-[0_4px_20px_rgba(0,0,0,0.06)] dark:shadow-[0_0_30px_rgba(47,123,255,0.12)]"
                      : "bg-transparent border-transparent hover:border-black/10 dark:hover:border-white/10 hover:bg-black/[0.02] dark:hover:bg-white/[0.02]"
                  }`}
                >
                  {/* Left accent bar on active */}
                  {isActive && (
                    <div className="absolute left-0 top-6 bottom-6 w-1 rounded-r bg-[#1964eb] dark:bg-[#2f7bff] shadow-[0_0_10px_#2f7bff]" />
                  )}
                  ...
                  <div className="flex items-center justify-between gap-4 mb-2 pl-2">
                    <div className="flex items-center gap-3">
                      <span
                        className={`font-mono text-xs font-semibold tracking-widest transition-colors ${
                          isActive ? "text-[#1964eb] dark:text-[#2f7bff]" : "text-zinc-500 group-hover:text-zinc-600 dark:group-hover:text-zinc-400"
                        }`}
                      >
                        {cap.number}
                      </span>
                      ...
                      <span
                        className={`font-mono text-[11px] uppercase tracking-wider px-2 py-0.5 rounded border transition-colors ${
                          isActive
                            ? "bg-[#1964eb]/10 dark:bg-[#2f7bff]/10 border-[#1964eb]/30 dark:border-[#2f7bff]/30 text-[#1964eb] dark:text-[#5b9aff]"
                    ...
                    <ArrowRight
                      className={`w-4 h-4 transition-all duration-300 ${
                        isActive
                          ? "text-[#1964eb] dark:text-[#5b9aff] translate-x-1"
                          : "text-zinc-400 dark:text-zinc-600 opacity-0 group-hover:opacity-100 group-hover:translate-x-0.5"
                      }`}
                    />
                  </div>
                  ...
                  <h3
                    className={`text-lg font-semibold tracking-tight transition-colors pl-2 ${
                      isActive ? "text-zinc-900 dark:text-white" : "text-zinc-600 dark:text-zinc-400 group-hover:text-zinc-900 dark:group-hover:text-zinc-200"
                    }`}
                  >
                    {cap.title}
                  </h3>
                  ...
                  <p
                    className={`text-xs mt-2 leading-relaxed transition-colors line-clamp-2 pl-2 ${
                      isActive ? "text-zinc-600 dark:text-zinc-300" : "text-zinc-500 group-hover:text-zinc-600 dark:group-hover:text-zinc-400"
                    }`}
                  >
                    {cap.tagline}
                  </p>
          ...
          <div
            ref={previewStageRef}
            className="col-span-7 p-8 sm:p-10 rounded-3xl bg-white dark:bg-[#09090d] border border-black/10 dark:border-white/10 space-y-8 shadow-[0_20px_50px_rgba(0,0,0,0.06)] dark:shadow-2xl relative overflow-hidden"
          >
            {/* Ambient inner glow */}
            <div className="absolute top-0 right-0 w-96 h-96 bg-[#1964eb]/5 dark:bg-[#2f7bff]/10 rounded-full blur-[100px] pointer-events-none" />

            {/* Showcase Visual Viewport */}
            <div className="rounded-2xl bg-[#f1f3f9] dark:bg-[#111116] border border-black/10 dark:border-white/10 overflow-hidden shadow-xl">
              <div className="flex items-center justify-between px-4 py-2.5 bg-[#e4e7f0] dark:bg-[#17171d] border-b border-black/5 dark:border-white/5">
                <div className="flex items-center gap-1.5">
                  <span className="w-2.5 h-2.5 rounded-full bg-red-500/60" />
                  <span className="w-2.5 h-2.5 rounded-full bg-yellow-500/60" />
                  <span className="w-2.5 h-2.5 rounded-full bg-green-500/60" />
                </div>
                <div className="flex items-center gap-2 px-2.5 py-0.5 rounded bg-white dark:bg-black/50 text-[11px] font-mono text-zinc-700 dark:text-zinc-400">
                  {activeCapability.isPrototype ? (
                    <Smartphone className="w-3 h-3 text-[#1964eb] dark:text-[#2f7bff]" />
                  ) : (
                    <Globe className="w-3 h-3 text-[#1964eb] dark:text-[#2f7bff]" />
                  )}
                  <span>{activeCapability.title}</span>
                </div>
                <div className="text-[10px] font-mono text-zinc-600 dark:text-zinc-500 uppercase">
                  {activeCapability.isPrototype ? "Mobile Concept" : "Production Web"}
                </div>
              </div>

              {activeCapability.isPrototype ? (
                <div className="relative w-full py-10 px-4 bg-[#f5f6fa] dark:bg-[#050507] flex items-center justify-center overflow-hidden">
                ...
                <div className="relative w-full aspect-[16/9] bg-[#f5f6fa] dark:bg-[#050507] overflow-hidden">
                  <Image
                    src={activeCapability.previewImage}
                    alt={activeCapability.title}
                    fill
            ...
            <div className="space-y-3 pt-2 border-t border-black/5 dark:border-white/5">
              ...
              <div className="grid grid-cols-2 gap-3">
                {activeCapability.services.map((svc, sIdx) => (
                  ...
                  >
                    <CheckCircle2 className="w-3.5 h-3.5 text-[#1964eb] dark:text-[#2f7bff] shrink-0 mt-0.5" />
                    <span>{svc}</span>
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-3 pt-2 border-t border-black/5 dark:border-white/5">
              ...
              <div className="flex flex-wrap gap-2">
                {activeCapability.technologies.map((tech) => (
                  <span
                    key={tech}
                    className="inline-flex items-center gap-1.5 px-3 py-1 rounded-md bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-xs font-mono text-zinc-700 dark:text-zinc-300"
                  >
                    <Cpu className="w-3 h-3 text-[#1964eb] dark:text-[#2f7bff]" />
                    <span>{tech}</span>
                  </span>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-2.5">
                {relatedProjects.map((p) => (
                  <Link
                    key={p.id}
                    href={`/work/${p.id}`}
                    className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-black/[0.04] dark:bg-white/[0.04] hover:bg-[#1964eb] dark:hover:bg-[#2f7bff] border border-black/10 dark:border-white/10 hover:border-[#1964eb] dark:hover:border-[#2f7bff] text-xs text-zinc-800 dark:text-white hover:text-white dark:hover:text-white transition-all duration-200 group"
                  >
                    <span className="font-mono text-[#1964eb] dark:text-[#5b9aff] group-hover:text-white">
                      {p.number}
                    </span>
        ...
        <div className="lg:hidden space-y-4">
          {CAPABILITIES.map((cap) => {
            ...
            return (
              <div
                key={cap.id}
                className="rounded-2xl bg-white dark:bg-[#09090d] border border-black/10 dark:border-white/10 overflow-hidden shadow-sm"
              >
                ...
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-xs font-semibold text-[#1964eb] dark:text-[#2f7bff]">
                        {cap.number}
                      </span>
                      <span className="text-zinc-400 dark:text-zinc-600 font-mono text-xs">/</span>
                      <span className="font-mono text-[10px] uppercase text-zinc-600 dark:text-zinc-400">
                  ...
                  <div className="w-8 h-8 rounded-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 flex items-center justify-center text-zinc-400">
                    {isExpanded ? (
                ...
                {isExpanded && (
                  <div
                    id={`content-${cap.id}`}
                    className="p-5 pt-0 space-y-5 border-t border-black/5 dark:border-white/5"
                  >
                    <p className="text-xs text-zinc-600 dark:text-zinc-300 leading-relaxed pt-3">
                      {cap.description}
                    </p>

                    {/* Preview Image */}
                    <div className="relative w-full aspect-[16/9] rounded-xl overflow-hidden bg-black/60 border border-black/10 dark:border-white/10">
                      <Image
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="space-y-1.5">
                        {cap.services.map((svc, sIdx) => (
                          ...
                          >
                            <CheckCircle2 className="w-3 h-3 text-[#1964eb] dark:text-[#2f7bff] shrink-0" />
                            <span>{svc}</span>
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {cap.technologies.map((t) => (
                          <span
                            key={t}
                            className="px-2 py-0.5 rounded bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-[11px] font-mono text-zinc-700 dark:text-zinc-300"
                          >
                            {t}
                          </span>
                        ))}
                      ...
                      <div className="flex flex-wrap gap-2">
                        {related.map((p) => (
                          <Link
                            key={p.id}
                            href={`/work/${p.id}`}
                            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-xs text-zinc-800 dark:text-white"
                          >
                            <span className="font-mono text-[#1964eb] dark:text-[#5b9aff]">{p.number}</span>
                            <span>{p.title}</span>
        ...
        <div className="mt-20 pt-10 border-t border-black/10 dark:border-white/10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div>
            <span className="text-xs font-mono text-zinc-500 uppercase tracking-wider block mb-1">
              Architecture &amp; Execution
            </span>
            <p className="text-lg font-medium text-zinc-900 dark:text-white tracking-tight">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-6 py-3.5 rounded-xl text-xs font-mono tracking-wider uppercase bg-black/5 dark:bg-white/5 hover:bg-[#1964eb] dark:hover:bg-[#2f7bff] text-zinc-700 dark:text-zinc-300 hover:text-white dark:hover:text-white border border-black/10 dark:border-white/10 hover:border-[#1964eb] dark:hover:border-[#2f7bff] transition-all duration-300 cursor-pointer group"
          >
